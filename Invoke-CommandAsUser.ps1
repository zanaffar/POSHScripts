## Runs a Script as the currently logged in user

## Usage ##

## Invoke-CommandAsUser -ComputerName '123XYZ','ABC123' -Credential 'domain\username' -DisableLogging -WriteHost $false
...
'@
                ...
                $lPath = 'C:\TempLogs\volume.log'
				#$netuse = & "cmd.exe" "/c net use"
                Write-Output "Mute? "([audio]::Mute) | Out-File -FilePath $lPath -Append
                Write-Output "Volume Level "([audio]::Volume) | Out-File -FilePath $lPath -Append
			...
			$lPath = 'C:\TempLogs\' + $env:COMPUTERNAME + '.log'
			$lPathVolume = 'C:\TempLogs\volume.log'

			Execute-ProcessAsUser -Path "powershell.exe" -Parameters "-encodedcommand $encodedcommand" -RunLevel LeastPrivilege -Wait
            #Execute-ProcessAsUser -Path "cmd.exe" -Parameters "/c net use" -Wait			

			$strResult = Get-Content -Path $lPath

            $objResult = [PSCustomObject]@{
                LoggedOnUserInfo = Get-LoggedOnUser
                Result = (Get-Content -Path $lPathVolume)
            }
