$height = 11
$Message = "Happy Holidays!!!`n"
$Message1 = "From Max!!`n`n"
$Message2 = "...can you tell that`n"
...
$Padding = ($Height * 2 - $Message1.Length + 1) / 2
if($Padding -gt 0){
    1..$Padding | % { Write-Host ' ' -NoNewline }
}
0..($Message1.Length -1) | %{
    $Index = $_
    switch ($Index % 2 ){
        0 { Write-Host -ForegroundColor Green $Message1[$Index] -NoNewline }
        1 { Write-Host -ForegroundColor Red $Message1[$Index] -NoNewline }
    }
}
