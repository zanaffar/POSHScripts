...
foreach($UDA_User in ($UDA_Users | Where-Object {($_ -notlike "$env:COMPUTERNAME\*")})) {
    ...
    if($User) {
        ...
        if((!($UDA_User -in (Get-LocalGroupMember -Group Administrators).Name))) {
            Write-Output "$UDA_User is not in the Administrators group. Adding $UDA_User to Administrators group." #| Out-File -FilePath $logfile -Append
            Add-LocalGroupMember -Group Administrators -Member $UDA_User
        } else { 
